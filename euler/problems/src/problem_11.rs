...
pub fn largest_product_in_grid() -> i64 {
    ...
    for row in 0..=20 - num {
        for col in 0..=20 - num {
            let horizontal_product = grid[row][col] * grid[row][col + 1] * grid[row][col + 2] * grid[row][col + 3];
            if horizontal_product > largest_product {
                largest_product = horizontal_product;
            }

            let vertical_product = grid[row][col] * grid[row + 1][col] * grid[row + 2][col] * grid[row + 3][col];
            if vertical_product > largest_product {
                largest_product = vertical_product;
            }


            let diagonal_down_product = grid[row][col] * grid[row + 1][col + 1] * grid[row + 2][col + 2] * grid[row + 3][col + 3];
            if diagonal_down_product > largest_product {
                largest_product = diagonal_down_product;
            }

            let diagonal_up_product = grid[row + 3][col] * grid[row + 2][col + 1] * grid[row + 1][col + 2] * grid[row][col + 3];
            if diagonal_up_product > largest_product {
                largest_product = diagonal_up_product;
            }
        }
    }
